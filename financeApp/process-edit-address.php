	...
	try{
		...
		if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            //email does not exists in a another record
                ...
                $query = "UPDATE  users SET  first_name=?, last_name=?, address1=?, address2=?, city=?, state_country=?, zcode_pcode=?, phone=? WHERE user_id=? LIMIT 1";
                //bind values to sql statements
                $q = mysqli_stmt_init($dbcon);
                mysqli_stmt_prepare($q, $query);
                mysqli_stmt_bind_param($q, 'ssssssssi', $first_nametrim, $last_nametrim, $address1trim, $address2trim, $citytrim, $state_countrytrim,$zcode_pcodetrim, $phonetrim, $id);
                //execute query
...
?>
            ...
            <h3 class="h3 text-center">Items marked with an asterik * are required</h3>
