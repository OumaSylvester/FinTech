<?php 
	try{
		...
		$query = "SELECT last_name, first_name, email, approved, DATE_FORMAT(registration_date, '%M %d, %Y') AS regdat, user_id FROM users WHERE user_level = 1 ORDER BY registration_date DESC LIMIT ?, ?";
		...
		if($result){//If it ran OK, display the records
			...
			while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
                ...
                if($approved == 'yes'){
                    ...
                    $_SESSION['pmanid'] = $user_id;
                    echo '<tr>
                    <td><a href="process-approve-project-manager.php">Approve</a></td>
                    <td><a href="edit-record.php?id='. $user_id .'">Edit</a></td>
                    <td><a href="delete-record.php?id=' . $user_id . '">Delete</a></td>';
		...
		if($pages > 1)
		{
			...
			if($current_page != 1)
			{
				$echostring .= '<a href="approve-project-manager.php?s=' . ($start - $pagerows) . '&p=' . $pages . '">Previous &nbsp;</a>';
			}

			//Create a next link
			if($current_page != $pages)
			{
				$echostring .= '<a href="approve-project-manager.php?s=' . ($start + $pagerows) . '&p=' . $pages . '">&nbsp;Next</a>';
			}
			$echostring .= '</p>';
			echo $echostring;
		}
